...
import { execute } from "./workerPool.js";
...
export const decodeEvents = async (
  ...
  common.logger.info({
    service: "events",
    msg: `Decoding ${rawEvents.length} events`,
  });

  const endClock = startClock();

  // Await the worker pool execution
  const events = await execute({ chunk: rawEvents, sources });
  // console.log('Events:', events);

  const duration = endClock();
  common.logger.info({
    service: "events",
    msg: `Decoded ${events.length} events in ${duration.toFixed(2)}s`,
  });
  common.metrics.ponder_indexing_abi_decoding_duration.observe(duration);
